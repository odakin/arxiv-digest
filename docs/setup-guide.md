...
scoring_threshold: 75           # Papers scoring >= this get delivered (0-100)
...
- **`scoring_threshold`**: Higher = fewer but more relevant papers. 75 is a good starting point
...
### Step 3: Create `interest_profile.txt`

This file tells Claude what papers matter to you. There are two paths:

#### Path A: HEP Researchers (INSPIRE-HEP)

If you have an INSPIRE-HEP author profile (BAI), you can auto-generate the profile:

1. Clone your new repo locally: `git clone https://github.com/YOU/my-arxiv-digest.git`
2. Install dependencies: `pip install -r requirements.txt`
3. Run: `python -m tools.setup_inspire YOUR.BAI.ID` (e.g. `K.Y.Oda.1`)
4. This fetches your publication history from INSPIRE and generates `interest_profile.txt`
5. Review and edit the generated file if needed
6. Commit and push: `git add interest_profile.txt && git commit -m "Add profile" && git push`
...
Edit `config.yaml` and `interest_profile.txt` as described in Mode A Steps 2-3 above.

For `interest_profile.txt`, you can use the INSPIRE auto-generation:

```bash
pip install -r requirements.txt
python -m tools.setup_inspire YOUR.BAI.ID
```

Or manually edit using the template at `templates/interest_profile.txt`.
...
1. Run `python -m src.fetch` to get new papers
2. Score each paper against your interest profile (Claude does this directly, no API call)
3. Run `python -m src.post` to deliver scored papers to your channels
...
- If you get no recommendations, try lowering `scoring_threshold` (e.g. from 75 to 60)
- If you get too many, raise it (e.g. from 75 to 85)
...
主な設定項目:
- **`language`**: 推薦文・要約の言語
- **`scoring_threshold`**: 高くすると厳選、低くすると多めに配信。75 が目安
...
### ステップ 3: `interest_profile.txt` を作成

このファイルで Claude にどんな論文が重要かを伝えます。2つの方法があります:

#### パス A: HEP 系研究者（INSPIRE-HEP）

INSPIRE-HEP に著者プロファイル（BAI）がある場合、自動生成できます:

1. リポをローカルにクローン: `git clone https://github.com/YOU/my-arxiv-digest.git`
2. 依存パッケージをインストール: `pip install -r requirements.txt`
3. 実行: `python -m tools.setup_inspire YOUR.BAI.ID`（例: `K.Y.Oda.1`）
4. INSPIRE から出版履歴を取得し、`interest_profile.txt` を自動生成
5. 生成されたファイルを確認・必要に応じて編集
6. コミット & プッシュ: `git add interest_profile.txt && git commit -m "Add profile" && git push`
...
モード A のステップ 2-3 と同じ手順で `config.yaml` と `interest_profile.txt` を編集。

`interest_profile.txt` は INSPIRE 自動生成も使えます:
...
または `templates/interest_profile.txt` を参考に手動編集。
...
1. `python -m src.fetch` で新着論文を取得
2. 興味プロファイルに基づいて各論文をスコアリング（Claude が直接実行、API 呼び出しなし）
3. `python -m src.post` でスコア結果をチャンネルに配信
...
- 推薦が来ない場合は `scoring_threshold` を下げてみる（例: 75 → 60）
- 多すぎる場合は上げる（例: 75 → 85）
