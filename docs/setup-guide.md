...
scoring_threshold: 85           # Papers scoring >= this get delivered (0-100)
...
- **`scoring_threshold`**: Higher = fewer but more relevant papers. 85 is a good starting point
...
- If you get no recommendations, try lowering `scoring_threshold` (e.g. from 85 to 60)
...
scoring_threshold: 85           # この点数以上の論文を配信（0-100）
...
主な設定項目:
- **`language`**: 推薦文・要約の言語
- **`scoring_threshold`**: 高くすると厳選、低くすると多めに配信。85 が目安
...
- 推薦が来ない場合は `scoring_threshold` を下げてみる（例: 85 → 60）
- 多すぎる場合は上げる（例: 80 → 90）
