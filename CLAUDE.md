...
**開発者（odakin）自身もこのリポで日常運用する**。physics-research の arxiv_digest はこちらに統合・移行する。
...
│                                                     │
│  1. python -m src.fetch → today_papers.json 出力     │
│  2. Claude 自身が JSON を読みスコアリング             │
│     （API 不使用、Pro Max OAuth で無料）               │
│  3. python -m src.publish → スコア結果をチャンネル配信 │
│                                                     │
...
- **パス A: INSPIRE 自動生成**（HEP 系研究者向け）
  1. `python -m tools.setup_inspire K.Y.Oda.1` を実行
...
| `arxiv_digest/run_digest.py` | `src/main.py` + `src/fetch.py` | モード A は main.py、モード B は fetch.py + post.py |
| `fetch_inspire.py` | `tools/fetch_inspire.py` | BAI を引数で受け取る |
| SKILL.md (scheduled task) | `skill/SKILL.md` | パスを arxiv-digest リポに変更、config.yaml 参照に |

#### 移行後の physics-research

- `papers/arxiv_digest/` ディレクトリを削除
- CLAUDE.md から arXiv ダイジェスト関連のセクションを削除
- scheduled task の SKILL.md のパスを `~/Claude/arxiv-digest/` に変更
- `papers/papers.yaml`, `papers/fetch_inspire.py` 等の論文データベース機能は physics-research に残す（arXiv ダイジェストとは独立した機能）

### 10. GitHub Actions ワークフロー設計（モード A）
...
### 11. Claude Code Scheduled Task 設計（モード B）
...
1. `cd ~/Claude/arxiv-digest && python -m src.fetch` を実行し、arXiv RSS から新着論文を取得
